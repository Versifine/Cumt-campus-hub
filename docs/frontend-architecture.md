...
- 时间展示统一为 UTC+8（北京时间），24 小时内显示相对时间，超过一天显示绝对时间。
...
### 评论区图片（RichContent variant="comment"）
- 不使用模糊背景，背景透明
- 小图片保持原始大小，左对齐
- 大图片限制最大高度 300px
- 点击可放大预览

### 媒体网格（CommentMediaBlock）
- 用于评论/帖子的附件媒体展示
...
### 相关组件
- `RichContent`：渲染富文本内容，支持 `variant` 属性区分帖子/评论
- `CommentMediaBlock`：媒体网格组件
- `MediaViewer`：全屏图片/视频查看器
